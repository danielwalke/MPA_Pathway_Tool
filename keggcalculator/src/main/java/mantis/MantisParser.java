			...
			for(MantisProtein protein : file.getMantisProteins()) {
				...
				for(int sequenceLineNumber = 0; sequenceLineNumber< numberOfSequenceLines; sequenceLineNumber++) {
					...
					if(endIndex> sequenceLength) {
						subSequence = protein.getSequence().substring(sequenceLineNumber*60, protein.getSequence().length());
					}
					if(endIndex <=sequenceLength) {
						subSequence = protein.getSequence().substring(sequenceLineNumber*60, (sequenceLineNumber+1)*60);
					}
