...
const NodeModificationBody = () => {
    ...
    const changeNodeSize = (e) => {
        ...
        dispatch({
            type: "ADD_NODE_MODIFICATION_TO_AUDIT_TRAIL", payload: {
    ...
    return (
        <div className={classes.paper} style={{width: "60vw", display: "grid", gap: "2px"}}>
            ...
            <div style={{border: "2px solid black"}}><h3>Color-Filter</h3>
                <div>matching string: <ToolTipBig title={"Enter a valid string or regular expression to filter nodes"}
                                                  placement={"right"}>
                    <TextField value={matchingString} onChange={(e) => setMatchingString(e.target.value)}/>
                </ToolTipBig></div>
                <div>filtered Color Change: <ToolTipBig
                    title={"Enter a valid color for changing color for filtered nodes"} placement={"right"}>
