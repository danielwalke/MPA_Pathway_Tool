...
const SbmlIdChanger = (props) => {
    ...
    return (
        <div>
            <div style={{display: "flex"}}><Checkbox checked={isChanged}/><TextField variant={"outlined"} size={"small"}
                                                                                     defaultValue={props.reaction.sbmlId}
                                                                                     value={reactionSbmlIds[props.index]}
                                                                                     onChange={(e) => {
            ...
            <button style={{width: 0.1 * window.innerWidth}} onClick={() => handleSubmitSbmlIdChanges()}
                    className={"downloadButton"}>submit changes
            </button>
        </div>
    );
};
