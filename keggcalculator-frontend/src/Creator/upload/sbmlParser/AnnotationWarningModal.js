...
const AnnotationWarningModal = () => {
    ...
    const annotationWarning = (<div className={classes.paper} style={{width:"45vw"}}>
        ...
        <div style={{display:"flex"}}>
            <button className={"downloadButton"} style={{width:"20vw"}} onClick={()=> handleAnnotationPurpose(dispatch)}>Yes</button>
            <button style={{width:"20vw"}} className={"downloadButton"} onClick={()=> handleSkipPurpose(state, dispatch)}>No</button>
        </div>
    </div>)
    return (
        <div>
