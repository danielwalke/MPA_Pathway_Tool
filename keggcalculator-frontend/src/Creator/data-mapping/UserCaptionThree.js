    ...
    return (
        <div className={"captionContainer3"}>
            <div className={"num"} style={{display: "grid", gridTemplateColumns:" repeat(5, 20%)", width:"94vw"}}>
                <div style={{justifySelf:"start"}}><input style={{width: "4vw"}} type={"number"}
                                               onChange={(e) => dispatch({
                ...
                <div ><input style={{width: "4vw"}} type={"number"}
                                               onChange={(e) => dispatch({
                                                   ...
                                                   payload: e.target.value
                                               })} value={proteinState.midQuantUser3}/></div>
                <div style={{justifySelf:"end"}}>{+(+mid + (+max - +mid) / 2).toFixed(numOfDigits)}</div>
                <div style={{justifySelf:"end"}}><input style={{width: "4vw"}} type={"number"}
                                               onChange={(e) => dispatch({
                                                   type: "SETMAXQUANTUSER3",
                                                   payload: e.target.value
                                               })} value={proteinState.maxQuantUser3}/></div>
            </div>
