...
const GraphVisualization_flux = () => {
    ...
    const fluxRate = (f) => {
        if(f.nodeId >=0 & f.nodeId <=100 ){
            return 1
        }
        else if(f.nodeId >100 & f.nodeId<=200){
            return 1.5
        }
        else if(f.nodeId>200 & f.nodeId<= 500){
            return 2
        }
        else if (f.nodeId<0){
            return 0.5
    ...
    const fluxForLinks = (reaction) => {
        if(reaction.fbaSolution = 0 & reaction.fbaSolution <=100){
            return "red";
        }
        else if(reaction.fbaSolution >= 100 & reaction.fbaSolution <= 300){
            return "yellow";
        }
        else if(reaction.fbaSolution >= 300 & reaction.fbaSolution <=500 ){
            return "green";
        }
        else if(reaction.fbaSolution >=500 & reaction.fbaSolution <=700){
            return "purple";
        }
        else{
            return "black";
    ...
    const myConfig = {
        ...
        link: {
            ...
            color: fluxForLinks(graphState.data),
            strokeWidth:fluxRate(graphState.data), //this will change accordingly to flux rate
            linkStrength: 5
