    ...
    const onModuleFileChange = async(event, dispatch, state) => {
        ...
        reader.onload = e => {
            ...
            try{
                ...
                const {nodes, links} = handleJSONGraphUpload(state.general.reactionsInSelectArray, dispatch, state.graph, state.general)
                const data = {nodes: nodes, links: links}
                dispatch({type: "SETDATA", payload: data})
                dispatch({type: "SWITCHISMODULEIMPORT"})
                dispatch({type: "SET_NEWDATALINKS", payload: links})
